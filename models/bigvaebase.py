...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 norm_layer=None, output_dim=2048):
        ...
        self.mu_logvar_gen = nn.Linear(512 * block.expansion, output_dim * 2)
    ...
    def _forward_impl(self, x):
        ...
        outputs = mu_logvar.view(batch_size, -1, 2).unbind(-1)
        # x = self.fc(x)
...
#         self.conv2 = nn.ConvTranspose2d(512, 64, 7, 2, 1)  # 4 x 4
#         self.bn2 = nn.BatchNorm2d(64)
#         self.conv3 = nn.ConvTranspose2d(64, 64, 6, 2, 1)  # 8 x 8
#         self.bn3 = nn.BatchNorm2d(64)
#         self.conv4 = nn.ConvTranspose2d(64, 32, 6, 2, 1)  # 16 x 16
#         self.bn4 = nn.BatchNorm2d(32)
#         self.conv5 = nn.ConvTranspose2d(32, 32, 6, 2, 1)  # 32 x 32
#         self.bn5 = nn.BatchNorm2d(32)
#         self.conv6 = nn.ConvTranspose2d(32, 32, 6, 2, 1)  # 32 x 32
#         self.bn6 = nn.BatchNorm2d(32)
#         self.conv_final = nn.ConvTranspose2d(32, 3, 6, 2, 0)
#
#         # setup the non-linearity
#         self.act = nn.LeakyReLU(inplace=True)
#
...
class ResDecoder(nn.Module):
    def __init__(self, input_dim):
        ...
        self.fc = nn.Linear(input_dim, 3 * 64 ** 2)
    ...
    def forward(self, z):
        # h = z.view(z.size(0), z.size(1), 1, 1)
        h = self.fc(z).view(z.size(0), 3, 64, 64)
        out = self.l1(h)
        out = self.resblocks(out)
        img_ = torch.sigmoid(self.l2(out))

        return img_


class BigVAEBase(nn.Module):
    def __init__(self, args):
        super(BigVAEBase, self).__init__()
        self.args = args
        self.domain_list = np.arange(args.num_domains)
        # create the encoder and decoder networks
        self.encoder_d = DomainEncoder(args.num_domains, args.domain_in_features)

        self.encoder_z = resnet50(output_dim=args.in_features)
    ...
    def forward(self, x, domain_labels, domain_id):
        ...
        """
        # fake_domains = np.delete(self.domain_list, domain_id)
