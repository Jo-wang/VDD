...
class Logger():
    ...
    def log_images(self, tag, img_tensor, global_step=None):
        self._writer.add_images(tag, self.nvar(img_tensor), global_step)

    def log_embed(self, embed, metadata, label_img=None, global_step=None):
        self._writer.add_embedding(self.nvar(embed), metadata=metadata, label_img=label_img, global_step=global_step)

    def log_hist(self, tag, hist, global_step=None):
        self._writer.add_histogram(tag, self.nvar(hist), global_step)
